...
async def narrow_and_pick(
    ...
    chunk_size: int = CHUNK_SIZE, k: int = BEAM_K, min_fit: float = 0.5, state_extra: dict | None = None,
) -> NarrowVerdict:
    ...
    if not shortlist:
        return decide(None, {}, 0.0, {}, candidates, min_fit=min_fit)
    ...
    return decide(pick.choice, pick.probabilities, pick.confidence, fits, candidates, min_fit=min_fit)
