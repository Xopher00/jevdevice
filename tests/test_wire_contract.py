"""The FROZEN wire contract, pinned: the question models now come from
typesymbolic (jev.py re-exports them), so these exact serialized shapes guard
the migration -- a core-side model change that alters any byte here must fail
this test before it can silently change what the judge is asked.
"""
...
def test_choice_wire_shape_keeps_undescribed_labels() -> None:
    ...
    # it must stay a JSON null on the wire, exactly as the pre-migration local
    # model serialized it.
