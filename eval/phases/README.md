...
(they hold machine-local data: device serials, hostnames, captured
distributions). `flywheel/TRAINING_FORMAT.md` is the exception: it documents
the training formats and stays tracked. The held-out half of `../goals.yaml`
is never run, tuned on, or exported.
