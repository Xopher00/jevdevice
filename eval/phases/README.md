...
One-off and per-phase evaluation scripts live here, isolated from the package.
Functional names, not phase numbers; LOGBOOK.md and agent-plans/ use the old names, which map as:
...
| audit_gate.py | — | gate-quality table (escalation + false-approval rates) from the journal |
| audit_arbitration.py | — | offline replay of matching.decide()/accept_any_fitting vs recorded distributions |
...
`cli/` — local run OUTPUTS, regenerated by these tools and not tracked in git
(machine-local data: device serials, hostnames, captured distributions).
`flywheel/TRAINING_FORMAT.md` is the tracked exception. The held-out half of
`../goals.yaml` is never run, tuned on, or exported.
