...
def main() -> int:
    directory = Path(sys.argv[1]) if len(sys.argv) > 1 else Path.home() / ".jevdevice" / "tsjournal"
    rows = [r for r in Journal(root=directory, rotation="daily",
            background_writes=False).replay() if r.get("type") == "decision"]
    if not rows:
    ...
    for row in rows:
        ...
        usage = (row.get("extra") or {}).get("usage") or {}
        ...
        truncated = bool((row.get("extra") or {}).get("truncation"))
        error = row.get("error")
